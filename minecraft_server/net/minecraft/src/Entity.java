...
public abstract class Entity
{
    ...
    public EnumEntitySize field_55074_am;
    ...
    public Entity(World par1World)
    {
        ...
        addedToChunk = false;
        field_55074_am = EnumEntitySize.SIZE_2;
        worldObj = par1World;
    ...
    protected void setSize(float par1, float par2)
    {
        ...
        if ((double)f < 0.375D)
        {
            field_55074_am = EnumEntitySize.SIZE_1;
        }
        ...
        {
            field_55074_am = EnumEntitySize.SIZE_3;
        }
        else if ((double)f < 1.375D)
        {
            field_55074_am = EnumEntitySize.SIZE_4;
        }
        else if ((double)f < 1.75D)
        {
            field_55074_am = EnumEntitySize.SIZE_5;
        }
        else
        {
            field_55074_am = EnumEntitySize.SIZE_6;
        }
    }
    ...
    public void func_55073_i(Entity par1Entity)
    {
        ...
        for (double d3 = -1.5D; d3 < 2D; d3 += 1.5D)
        {
            for (double d4 = -1.5D; d4 < 2D; d4 += 1.5D)
            {
                ...
                if (!worldObj.func_55199_a(axisalignedbb).isEmpty())
                {
                    continue;
                }

                if (worldObj.func_58038_s(i, (int)posY, j))
                {
                    setLocationAndAngles(posX + d3, posY + 1.0D, posZ + d4, rotationYaw, rotationPitch);
                    return;
                }

                if (worldObj.func_58038_s(i, (int)posY - 1, j) || worldObj.getBlockMaterial(i, (int)posY - 1, j) == Material.water)
                {
                    d = posX + d3;
                    d1 = posY + 1.0D;
                    d2 = posZ + d4;
    ...
    public String toString()
    {
        return String.format("%s['%s'/%d, l='%s', x=%.2f, y=%.2f, z=%.2f]", new Object[]
                {
                    getClass().getSimpleName(), getUsername(), Integer.valueOf(entityId), worldObj != null ? worldObj.getWorldInfo().func_55310_h() : "~NULL~", Double.valueOf(posX), Double.valueOf(posY), Double.valueOf(posZ)
                });
    }
}
