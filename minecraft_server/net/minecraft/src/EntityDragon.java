...
public class EntityDragon extends EntityDragonBase
{
    ...
    public double[] func_40139_a(int par1, float par2)
    {
        ...
        par2 = 1.0F - par2;
        int i = field_40145_e - par1 * 1 & 0x3f;
        int j = field_40145_e - par1 * 1 - 1 & 0x3f;
        ...
        ad[2] = field_40144_d[i][2] + (field_40144_d[j][2] - field_40144_d[i][2]) * (double)par2;
        return ad;
    }
    ...
    public void onLivingUpdate()
    {
        ...
        else
        {
            ...
            Vec3 vec3 = Vec3.func_58052_a().func_58076_a(targetX - posX, targetY - posY, targetZ - posZ).normalize();
            Vec3 vec3_1 = Vec3.func_58052_a().func_58076_a(MathHelper.sin((rotationYaw * (float)Math.PI) / 180F), motionY, -MathHelper.cos((rotationYaw * (float)Math.PI) / 180F)).normalize();
            float f18 = (float)(vec3_1.dotProduct(vec3) + 0.5D) / 1.5F;
            ...
            Vec3 vec3_2 = Vec3.func_58052_a().func_58076_a(motionX, motionY, motionZ).normalize();
            float f22 = (float)(vec3_2.dotProduct(vec3_1) + 1.0D) / 2.0F;
            f22 = 0.8F + 0.15F * f22;
            motionX *= f22;
            motionZ *= f22;
        ...
        float f3 = (((float)(func_40139_a(5, 1.0F)[1] - func_40139_a(10, 1.0F)[1]) * 10F) / 180F) * (float)Math.PI;
        ...
        if (!worldObj.isRemote)
        {
            field_40159_q = destroyBlocksInAABB(dragonPartHead.boundingBox) | destroyBlocksInAABB(dragonPartBody.boundingBox);
        }
    }
    ...
    protected void onDeathUpdate()
    {
        ...
        if (!worldObj.isRemote && field_40158_r > 150 && field_40158_r % 5 == 0)
        {
            for (int i = 1000; i > 0;)
            {
        ...
        if (field_40158_r == 200 && !worldObj.isRemote)
        {
            for (int j = 2000; j > 0;)
            {
